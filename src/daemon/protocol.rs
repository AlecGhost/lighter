...
use super::{Error, RequestOptions, Result};
    ...
    E: std::fmt::Display,
{
    ...
    match highlight(&request.language, &source, &request.options) {
...
pub(super) fn exchange(
    ...
    write_request(&mut *stream, id, language, source.as_bytes(), options)?;
...
mod tests {
    ...
    fn request_handler_returns_highlight_errors_without_a_body() {
        ...
        handle_request(
            request(VERSION),
            &mut output,
            &mut |_language, _source, _options| Err::<String, _>(HIGHLIGHT_ERROR),
        )
        .unwrap();
    ...
    fn request_handler_rejects_unsupported_versions() {
        ...
        handle_request(
            ...
            &mut output,
            &mut |_language, _source, _options| {
